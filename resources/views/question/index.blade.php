        ...
        <div class="flex items-center gap-3">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M9 12h6m2 6H7a2 2 0 01-2-2V6a2 2 0 012-2h6l6 6v10a2 2 0 01-2 2z" />
            </svg>
            ...
            <table class="min-w-full text-sm text-left text-gray-800">
                <thead class="bg-gray-100 border-b text-xs uppercase text-gray-500">
                    <tr>
                        ...
                        <th class="px-6 py-3">Answers</th> {{-- NEW --}}
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-200">
                    @foreach ($questions as $index => $question)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-indigo-600 font-semibold">{{ $question->quiz->title }}</td>
                            <td class="px-6 py-4">{{ Str::limit($question->question, 100) }}</td>
                            <td class="px-6 py-4">{{ $question->created_at->format('M d, Y') }}</td>
                            ...
                            <td class="px-6 py-4">
                                <a href="{{ route('questions.answers.index', $question->id) }}"
                                class="inline-block px-3 py-1 bg-green-600 text-white rounded text-xs hover:bg-green-700 transition">
                                Manage Answers
                                </a>
                            </td>
                        </tr>
